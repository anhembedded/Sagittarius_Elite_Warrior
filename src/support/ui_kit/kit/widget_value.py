...
def connect_value_committed(widget: QWidget, slot: Callable[[], None]) -> bool:
    ...
    * **Everything else** commits on its USER property's notify signal.
    ...
    # the same name is what can actually be connected. `QByteArray.data()` is
    # declared as `bytes | bytearray | memoryview`, and only the first of the
    # three has `.decode()` — `bytes(...)` is what makes the conversion hold
    # for all of them rather than for the one PySide6 happens to hand back
    # today.
    signal_name = bytes(user_property.notifySignal().name().data()).decode()
