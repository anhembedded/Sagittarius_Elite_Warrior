...
class _TabButton(QPushButton):  # base-exempt: a tab is a button, not a surface
    ...
    def __init__(self, tab: Tab, parent: QWidget | None = None) -> None:
        ...
        # Held rather than re-read through `self.layout()`: the two size
        # hints below are the only callers, and `QWidget.layout()` is typed
        # `QLayout | None` — correctly, since a widget need not have one.
        # Keeping the layout this constructor built means the hints cannot be
        # asked of `None`, which is what mypy reported the moment `EPIC-025`
        # PR 1.6b moved this file out of the `presentation/` exclusion.
        self._row = QHBoxLayout(self)
        self._label = QLabel(tab.label)
        self._row.addWidget(self._label)

        self._badge = Badge(tab.badge)
        self._row.addWidget(self._badge)
