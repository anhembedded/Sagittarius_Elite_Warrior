...
_SUPPORTED_CHART_TYPES = frozenset({"candlestick", "line"})


class NativeUnsupportedFeatureError(RuntimeError):
    """Raised when the caller asked the native host for something outside
    this slice's scope (script regions/info, arbitrary script markers).
    BackTestView must catch this and rebuild with the Python host — it is not a crash,
    it is the explicit "this content needs Python" signal."""
...
class NativeBacktestChartHostAdapter:
    ...
    def __init__(self, symbol: str, native_host: NativeBacktestChartHost) -> None:
        ...
        self._indicator_visibility: dict[str, bool] = {}
    ...
    def set_chart_type(self, chart_type: str) -> None:
        if chart_type not in _SUPPORTED_CHART_TYPES:
            raise NativeUnsupportedFeatureError(
                f"native chart type support is candlestick/line only, got {chart_type!r}"
            )
        self._chart_type = chart_type
        if self._pending_candles is not None and chart_type == "line":
            candles, self._pending_candles = self._pending_candles, None
            accepted = self._native_host.submit_ohlcv(
                candles,
                [],
            ...
            if not accepted:
                raise NativeUnsupportedFeatureError(
                    "native line-series OHLCV submission was rejected"
                )
    ...
    def add_subplot_indicator(
        ...
        # BOT-098F6F: renders equity/subplot indicator line via native indicator series
        self._indicator_series[name] = (_color_to_rgba(color), [], [])
        self._indicator_visibility[name] = True
    ...
    def set_indicator_visible(self, name: str, visible: bool) -> None:
        # BOT-098F6F: native per-indicator visibility toggling
        if name in self._indicator_series:
            self._indicator_visibility[name] = bool(visible)
            self._resubmit_indicators()

    def remove_indicator(self, name: str) -> None:
        self._indicator_visibility.pop(name, None)
    ...
    def _resubmit_indicators(self) -> None:
        series = [
            (rgba, x_data, y_data)
            for name, (rgba, x_data, y_data) in self._indicator_series.items()
            if x_data and y_data and self._indicator_visibility.get(name, True)
        ]
