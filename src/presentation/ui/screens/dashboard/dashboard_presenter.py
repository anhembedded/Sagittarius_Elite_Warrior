...
class DashboardPresenter(BasePresenter):
    ...
    def __init__(self, view: "DashboardView", app: "App"):
        ...
        # Important: Call the lifecycle hooks manually at the end of __init__
            ...
            try:
                ...
                if not klines:
                    self.ui_log_signal.emit(
                        f"No historical data found for {card.symbol}."
                    )
                    continue
                ...
                mapped_data = [
                    (
                        float(
                            item.close_time.timestamp()
                        ),  # Dùng close_time giống tick event
                ...
                self.ui_log_signal.emit(
                    f"Rendered {len(mapped_data)} historical klines for {card.symbol}."
                )

            except Exception as e:
                self.ui_log_signal.emit(
                    f"Exception while loading history for {card.symbol}: {str(e)}"
                )
    ...
    def _handle_market_tick(self, event: MarketTickEvent):
        ...
        # Chỉ bắn log lên System Monitor khi 1 cây nến đã ĐÓNG (VD: 1 phút 1 lần)
        # Bắn log liên tục mỗi tick sẽ làm treo cứng (Freeze) giao diện UI.
        if is_closed:
            self.ui_log_signal.emit(f"[Live] {symbol} candle closed at {c}")
