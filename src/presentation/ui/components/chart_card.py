    ...
    def dataBounds(self, ax, frac=1.0, orthoRange=None):
        ...
        if ax == 0:
            ...
            if orthoRange is not None:
                ...
                visible_lows = [
                    l for (t, o, h, l, c) in self.history_data if min_x <= t <= max_x
                ]
                visible_highs = [
                    h for (t, o, h, l, c) in self.history_data if min_x <= t <= max_x
                ]
...
class ChartCard(BaseCard):
    ...
    def _setup_content(self):
        ...
        self.main_plot = self.layout_widget.addPlot(
            row=1, col=0, axisItems={"bottom": date_axis}
        )
    ...
    def add_subplot_indicator(
        ...
        self.layout_widget.ci.layout.setRowStretchFactor(
            self._current_row, height_ratio
        )
