...
class MarketDataReportingContract:
    ...
    def test_status_of_an_empty_series_is_zeroed_not_absent(
        ...
        status = impl.get_database_status(SPOT, "NOSUCHCOIN", MINUTE)
    ...
    def test_status_reports_the_span_and_the_count(
        ...
        status = impl.get_database_status(SPOT, "BTCUSDT", MINUTE)
    ...
    def test_gaps_counts_discontinuities_not_missing_candles(
        ...
        impl.save_klines(SPOT, [candle(minutes=m) for m in (0, 1, 5)])

        assert impl.get_database_status(SPOT, "BTCUSDT", MINUTE).gaps == 1

    def test_a_contiguous_series_has_no_gaps(self, impl: IMarketDataRepository) -> None:
        impl.save_klines(SPOT, [candle(minutes=m) for m in range(10)])

        assert impl.get_gaps(SPOT, "BTCUSDT", MINUTE) == []
    ...
    def test_a_gap_names_its_boundaries_and_how_many_are_missing(
        ...
        impl.save_klines(SPOT, [candle(minutes=0), candle(minutes=6)])

        gaps = impl.get_gaps(SPOT, "BTCUSDT", MINUTE)
    ...
    def test_status_for_intervals_answers_every_interval_asked_for(
        ...
        missing key (`BUG-078`)."""
        impl.save_klines(SPOT, [candle(minutes=0)])

        statuses = impl.get_database_status_for_intervals(
            SPOT, "BTCUSDT", [MINUTE, TimeFrame.FIVE_MINUTES]
        )
    ...
    def test_status_for_intervals_agrees_with_asking_one_at_a_time(
        ...
        batch = impl.get_database_status_for_intervals(SPOT, "BTCUSDT", [MINUTE])

        assert batch[MINUTE.value] == impl.get_database_status(SPOT, "BTCUSDT", MINUTE)
    ...
    def test_range_coverage_of_an_empty_range_is_zeroed(
        ...
        snapshot = impl.get_range_coverage(
            SPOT, "NOSUCHCOIN", MINUTE, at(0), at(10), at(20)
        )
    ...
    def test_range_coverage_counts_a_still_forming_candle(
        ...
        impl.save_klines(SPOT, [candle(minutes=0), candle(minutes=1, closed=False)])

        snapshot = impl.get_range_coverage(SPOT, "BTCUSDT", MINUTE, at(0), at(5), at(2))
    ...
    def test_range_coverage_with_no_start_time_begins_at_the_oldest_candle(
        ...
        impl.save_klines(SPOT, [candle(minutes=m) for m in (2, 3, 4)])

        snapshot = impl.get_range_coverage(SPOT, "BTCUSDT", MINUTE, None, at(5), at(10))
