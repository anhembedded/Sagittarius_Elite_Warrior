...
This one implements all sixteen methods and passes
...
**Storage**: one dict per `(symbol, interval)` keyed by `open_time`. Upsert is
then a plain assignment, and ordering is `sorted()` over the keys — the two
things the real repository gets from a primary key and an `ORDER BY`.
"""
...
#: `(symbol, interval value)` -> `{open_time: candle}`.
...
class FakeMarketDataRepository(IMarketDataRepository):
    ...
    def __init__(self, klines: list[MarketData] | None = None) -> None:
        """`klines` seeds the store exactly as `save_klines()` would, so a
        consumer's test arranges its history in one line."""
        ...
        #: give it nothing.
        self.vacuum_calls: list[str | None] = []
        if klines:
    ...
    def clear_klines(self, symbol: str, interval: TimeFrame | None = None) -> int:
        ...
        for key in list(self._series):
            if key[0] != symbol:
                continue
            if interval is not None and key[1] != _interval_value(interval):
                continue
    ...
    def vacuum(self, symbol: str | None = None) -> None:
        self.vacuum_calls.append(symbol)
    ...
    def has_any_klines(self, symbol: str) -> bool:
        ...
        return any(key[0] == symbol and series for key, series in self._series.items())

    def list_available_shards(self) -> list[str]:
        return sorted({key[0] for key, series in self._series.items() if series})
    ...
    def get_database_status(
        ...
        rows = self._rows(symbol, interval)
        if not rows:
    ...
    def get_range_coverage(
        ...
        rows = [
            row
            for row in self._rows(symbol, interval)
