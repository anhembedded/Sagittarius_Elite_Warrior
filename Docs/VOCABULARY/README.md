...
| **`BoundedContextModule`** | The application's module base class: the Engine's `IExtension` plus three hooks of this app's own — `contribute()` and `subscribe()` for the UI, and `declare_cli()` for the interactive shell's commands. All three default to doing nothing. | `core/contracts` | HLD §3.1, SDD-01 | the Engine's legacy `IModule`; `AbstractScreenModule` (a screen, from `EPIC-016`) |
...
| **Empty by design** (`EMPTY_BY_DESIGN`) | One named `(guard, root, pattern)` triple whose scan finding **nothing** is the rule being satisfied rather than the guard having lost its subject. Introduced `EPIC-025` PR 4.3l for the `.qml` ban: with ADR D21 met there is no `.qml` under `src/` to find, while every *other* empty scan still fails. It is not the exemption the row above warns against — it exempts one scan from the emptiness check only, never from its own rule, and a second test asserts the triple is a scan that is really registered, so the exemption cannot outlive what it exempts. | `tests/unit/architecture/scanned_roots_registry.py` | `EPIC-025E` §4.12 | an allowlist ratchet, which records violations; this records an expected absence |
| **Symbol lease** | `trading`'s record that one `owner_id` currently holds a symbol: it refuses a second claimer and refuses **orders** from any other owner (`ExecuteOrderSafetyGate.SYMBOL_LEASED`). `strategy` claims on arm and releases on disarm; a manual order carries `owner_id="manual"`, `OrderRequest`'s default. The holder is not exposed on the public port. **Shipped `EPIC-025` PR 2.1f**, with three things measured rather than as first written: `claim_symbol` answers a `bool` instead of raising a `SymbolAlreadyLeased` (a refusal is a value, like every other refusal in this module); it is **one symbol per owner**, so re-arming onto another symbol gives the first back rather than stranding it; and it is claimed by *arming*, never by enabling trading — a lease tied to the switch would be released by an Emergency Stop, exactly when an open position most needs its strategy to still own the symbol. In-process, like the rest of the session's state. | `trading` | HLD §3.4, SDD-04 | the two mechanisms below, which have different semantics |
...
| :--- | :--- | :--- | :--- |
| **Surface** (= a **mode**) | A screen with **no business logic**: a nested `QMainWindow` (HLD §11) that asks the registry who contributes into each of its places and builds the widgets through the factories it is given; its dock layout is a **perspective** saved per user. Shell-owned surfaces: `welcome`, `trading`, `dev_board`, `settings`. | — | 🔵 (today `screens/trading` and `screens/dashboard` are ordinary screens) |
| **Module-owned screen** | A screen a module owns because it hosts a workflow the user performs for its own sake (§4.6.2 question 1): `backtest`, `data_management`. It is still a mode (`QMainWindow`), and declares which places accept foreign contributions (`accepts=(RAIL, MODAL)`). | the owning module | ✅ (`ScreenRegistry`) |
| **Place** | A named slot a module may contribute a widget to. The full list is the rows below. | — | — |
| `SCREEN` | A navigation entry that opens a full page — a **mode** in the mode selector. | the shell (`QStackedWidget` of nested `QMainWindow`s) | ✅ `ScreenRegistry`, `NavLocation` |
...
| `SETTINGS_SECTION` | A page in the Settings dialog (section list left, pages right, Apply / Cancel) for one module's own configuration keys. | the settings dialog — one page per module, by `order` | 🔵 |
| `STATUS_TILE` | A one-glance indicator (websocket pill, price ticker, run progress) in the `QStatusBar`. | the status bar | 🔵 |
| `DEV_PROBE` | A probe panel in Dev Board's dock area, only when `dev.mode` was true at boot. | the Dev Board dock area | ✅ |
| **Panel** | A `QDockWidget` in a dock area whose content is one module-owned widget (a table on a model, a form, a summary); title and close button and nothing else of its own; no reference to sibling panels. Replaces the **card** (retired 2026-09-13, ADR D22 — *"các card cũ cũng rất là tệ"*). | the owning module | 🔵 |
| **Dialog** | A `QDialog` for an action that needs input and confirmation (place an order — F9 —, arm a strategy, pick a range, edit settings): names what OK does, has Cancel, validates before OK enables, reports in the status bar. | the owning module | 🔵 |
| **Mode** / **Perspective** | A mode is a top-level screen selected in the sidebar (Qt Creator's term); its perspective is the user's dock layout for it, saved with `QMainWindow.saveState()` per mode and restored on return; "Reset layout" restores the contributed order. | shell | 🔵 |
| **Native theme** | The app applies no stylesheet, palette, token or third-party theme; widgets look like the OS's. Colour appears only where it carries meaning (profit/loss, connection). A designed theme is deferred (ADR D21). | shell | 🔵 |
...
| **Contribution kind** | What a contribution *is* for the shell: `screen`, `surface_widget`, `settings_section`, `status_tile`, `dev_probe`. Kinds are application policy, never Engine mechanism. | shell | HLD §4.3 | a place (where it goes) |
| **CLI command declaration** | What `module.declare_cli()` hands the shell: a frozen `CliCommandDescriptor` (`name`, `handler`, `contributor_id`) collected by `shell/cli_registry.py`. A **separate** registry from `ContributionRegistry`, and HLD §4.3's sixth contribution kind `cli_command` is therefore **not** how this shipped (`EPIC-025` PR 1.3c-5): a command has no surface, no place, no order and no widget factory, so putting it through `ContributionDescriptor` would have meant inventing a surface for it. What the two registries share is the inversion and the file shape, not the descriptor. | the declaring module | SDD §2 | a `ContributionDescriptor`; the argument spec, which stays in `cli_commands.json` |
...
| **Panel** (as a runtime object) | What a factory returns: a View and its Presenter built together; the Presenter owns its Coordinators and its action ownership. Two surfaces get two panels from one factory; they share the module's feed, never an object. | the contributing module | SDD "Ownership" | a shared singleton |
| **Mirror rule** | A `trading.rail` panel is also a `dev_board.rail` panel unless the module says otherwise; the reverse is not true. Same factory, two instances. | — | HLD §4.6.3 rule 2 | copying a widget |
| **Probe** (`dev_probe`) | A widget a module contributes to Dev Board to answer *what is actually true inside me right now* — either by trying an exchange API it does not yet understand, through the module's real adapter and showing raw request and response, or by showing the module's own state, which is what the first one built does (`trading`'s session probe, PR 1.4c-4: the live-submission flag, the session's order count and the symbols it believes open). Deleted or kept deliberately once the use case has tests. | the contributing module | HLD §4.4 | a feature |
...
| **Order quantity rounding** (`OrderQuantityRoundingPolicy`, `NotionalCheck`) | `trading`'s half of building an order the venue will accept: a quantity rounded **down** to the lot step, a price rounded to the tick in the direction that favours a fill, and whether the result clears the symbol's minimum notional. A filter, never a judgement about whether to trade — that is the **trading session**'s limits. Published since `EPIC-025` PR 2.1d, because `strategy`'s sizing bridge and the CLI's order-preview formatter both name it, and `OrderPreview` already carried a `NotionalCheck` field. | `modules/trading/contracts/order_quantity_rounding_policy.py` |
| **Trading limits** (`TradingLimits`, `TradingLimitViolation`, `TradingLimitContext`, `TradingLimitCheck`) | The four session limits an order is judged against — orders per session, notional per order, positions per symbol, minimum interval — as the thresholds (`TradingLimits`), the live numbers (`TradingLimitContext`), each verdict (`TradingLimitCheck`) and which one refused (`TradingLimitViolation`). All four are answers and are published; `TradingLimitPolicy`, which **decides**, is not, and stays in `domain/policies/`. Published since `EPIC-025` PR 2.1g, because `ExecuteOrderResult` had carried all four since PR 1.3b. | `modules/trading/contracts/trading_limits.py` |

### `strategy`

| Term | Definition | Defined in |
| :--- | :--- | :--- |
| **Strategy** | A rule that turns a candle buffer and a `StrategyContext` into a `Signal`. Listed by `StrategyRegistry`, which is **not** yet behind a port: `IStrategyCatalog` is planned (HLD §3.4) and `EPIC-025` PR 2.1c measured that it would have had no consumer it could serve, because every caller that reads the keys also needs the strategy classes. | `modules/strategy/domain/strategies` (since `EPIC-025` PR 2.1b) |
...
| **Armed strategy** (`ArmedStrategySnapshot`) | What is armed **right now**, as one atomic answer: the `LiveStrategyConfig` the user armed, and whether an engine is actually running from it. Two fields because they are two facts — arming records the value and then builds the engine — and one call because a reader deciding whether a strategy owns a symbol must not be able to see them disagree. Read through `IArmedStrategy.armed()`; nothing outside the module may arm, which goes through `ArmStrategyCommand`. | `contracts/i_armed_strategy.py`, `contracts/armed_strategy_snapshot.py` (since `EPIC-025` PR 2.1c) |
...
| **Strategy engine** (`IStrategyEngine`) | The object that feeds candles to a strategy and emits `SignalGeneratedEvent`; built by `IStrategyEngineFactory`, published as an interface since PR 3.1b so a consumer never names the class. Two methods, and they are **not** interchangeable: `on_tick()` takes a **closed** candle and commits the indicators, so batch and incremental runs over the same data cannot disagree; `on_forming_bar_tick()` takes one still **forming** and only peeks, so it may decide something the closed bar would not (`BOT-042D`). | `contracts/i_strategy_engine.py` |
| **Tick path** | The chain one closed candle travels from the exchange to an order: `market_data` emits `MarketTickEvent` on the websocket thread → `MarketTickEventHandler` (this context's, since `EPIC-025` PR 2.1c-2) → `LiveStrategySession.dispatch_tick()` filters to the armed symbol and interval → `StrategyEngine.on_tick()` → `LiveTradingCoordinator` → `trading`'s `IOrderSubmission`. It runs on the emitting thread from end to end and never touches Qt, which is why the subscription is made in `StrategyModule.boot()` rather than through `subscribe(bridge)` (`Docs/SDD/03` — threading contract for ports). | `application/event_handlers/` |
| **Sizing policy** (`ISizingPolicy`) | The rule that turns account equity, free balance, the strategy's sizing percent and leverage into the capital one order may use. Owned by `strategy` (ADR D17): "how much to bet" is a strategy decision; `trading` only rounds to the exchange's filters and enforces `TradingLimitPolicy`; `backtesting` sizes a paper fill by the same call, which is what makes a backtested size and a live size one number. **Not** a quantity — see `MarginAllocation` below and **Order quantity rounding**, in `trading`'s section. | `contracts/i_sizing_policy.py` (`EPIC-025` PR 2.1d) |
| **Margin allocation** (`MarginAllocation`) | What `ISizingPolicy` answers: the `margin` an order locks against the account and the `notional_capital` that margin buys at the armed leverage. Two numbers, not one, because leverage separates them — and `is_fundable` is the named decision *"this can pay for an order"*, which asks about both, since clamping the margin to a thin free balance can scale the notional to nothing. Not a quantity and not an order: the step from capital to a quantity needs the symbol's lot filter, which is `trading`'s. | `contracts/i_sizing_policy.py` |
| **Strategy card** (`StrategyCardViewModel`) | The card a live surface shows for *what to run*: which strategy, on which timeframe, at what sizing and leverage, its parameter form, what is armed right now, and the last signal. One class owned by `modules/strategy/ui`, one instance per surface — Trading and the Dev Board each have their own selection, and two screens writing one card's state would make "what did I pick here" unanswerable. Picking is **not** arming: the card records a choice, and `ArmStrategyCommand` is what acts on it. | `modules/strategy/ui/strategy_card_view_model.py` (`EPIC-025` PR 2.1e) |
...
| **Documentation-only change** | A change whose every path is in the set `.claude/ONBOARDING.md` §7 defines (the one definition); such a change may be committed, pushed and merged without asking. | `ONBOARDING.md` §7 |
| **Use-case spec** (`SPEC-xxx`) | One file describing one thing a person does with the app: trigger, preconditions, flow, what must be true afterwards, the named failures, what it does **not** promise, the ports it crosses, and the evidence that proves each part. Ids are permanent. It is what a phase or a pull request is accepted against — not prose about intent. | `Docs/SPEC/README.md` |
| **User story** (`US-xx`) vs **spec** (`SPEC-xxx`) | A `US` is something somebody **wants**, recorded as a proposal with acceptance criteria. A `SPEC` is what the app **does**, or is specified to do, with the flow spelled out and the evidence named. A `US` becomes a `SPEC` when it is specified; neither file copies the other, and the only link is the SPEC's *Origin* line. | `Tasks/UserStory_Propose.md` · `Docs/SPEC/README.md` |
| **Proven by** | A use-case spec's evidence table: for each promise, the test path that holds it (the path must exist — a guard checks) or the words **the user runs it** with what a person must see. Naming the human steps is how they stop being forgotten. | `Docs/SPEC/SPEC-000_template.md` §8 |
