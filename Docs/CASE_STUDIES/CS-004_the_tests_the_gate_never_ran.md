...
| :--- | :--- | :--- |
| the subject's own 13 unit tests | they lived beside it **under `src/`**, and the gate runs `pytest Sagittarius_Elite_Warrior/tests` — collected by nobody, on any run. PR 1.4b-2 found this and counted them; this is the first time it cost anything | **no — 0 files** since `EPIC-025` PR 4.3l; it was 22 when this was written |
...
- `tests/unit/architecture/test_no_test_file_lives_under_src.py` — **new**: no `test_*.py`,
  ...
  at once; the 22 sat in QML packages ADR D21 deletes, so the list reached **zero** at PR 4.3l.
...
- ~~**22 files** under `src/**/tests/` are still unrunnable.~~ **Closed at PR 4.3l** with the last
  QML package, 44 test functions restated. It fell by deletion every time, never by a moved test.
- Any docstring citing coverage by **location** rather than by a gate run makes the same claim.
- `src/presentation/` is outside mypy wholesale — unwatched until a file leaves it (PR 2.1e and
  PR 4.1b each found a real type error that way).
