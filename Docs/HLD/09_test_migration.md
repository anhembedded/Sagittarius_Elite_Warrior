...
| 4 | `unit/presentation/ui/{kit,qml,components,common,state,registry}` → `support/ui_kit` and `support/charting` mirrors (about 90 files; 2 went early in PR 1.6a with `assets/` — `test_icon_loader.py` and `test_asset_validator_extension.py` → `unit/support/ui_kit/assets/` — and the whole `unit/presentation/ui/kit/` tree in PR 1.6b, because §9.3's mirroring rule follows the subject, not the phase number. `test_palette_is_the_only_color_source.py` did **not** move: it is a scanning guard over *both* trees for as long as both exist) | — | — | the UI-map guard |
