...
| Tests of code the plan deletes (`RunBacktestCommand`, `BacktestState`, the two order events, `StatGrid`) | 5 | delete category — **PR 3.1a** took the first three: `test_run_backtest.py` (3 tests) and `test_stop_backtest.py` (1) went with their subject, and the assertion inventory is in `EPIC-025D` §3.2. Each asserted the replay loop itself (one `MarketTickEvent` per candle, the `time.sleep` throttle, the `is_running` flag), so nothing they proved survives the deletion by design; the "no data" case is covered better by `test_run_static_backtest.py::test_no_historical_data_emits_failed_event_and_returns_none`, which asserts a **named failure** rather than the absence of a publish |
