...
| Tests of code the plan deletes (`RunBacktestCommand`, `BacktestState`, the two order events, `StatGrid`) | 5 | delete category — **PR 3.1a** deleted **2 files / 4 tests** of this row's share: `test_run_backtest.py` (3 tests, `RunBacktestCommand` + `BacktestState`) and `test_stop_backtest.py` (1 test), which covers `StopBacktestCommand` as well — a subject this row never listed although it was deleted in the same breath, and the row is corrected here rather than left to imply it survived. The two order events and `StatGrid` are still open. Both files went with their subject, and the assertion inventory is in `EPIC-025D` §3.2. Each asserted the replay loop itself (one `MarketTickEvent` per candle, the `time.sleep` throttle, the `is_running` flag), so nothing they proved survives the deletion by design; the "no data" case is covered better by `test_run_static_backtest.py::test_no_historical_data_emits_failed_event_and_returns_none`, which asserts a **named failure** rather than the absence of a publish |
...
| 2 | `unit/modules/strategy/**` — `unit/domain/strategies`, the strategy services and arm/disarm **moved there by PR 2.1b**, tier unchanged (ADR D7), plus the 15 `LiveStrategySession` files | strategy card and last-signal card tests from the inventory | — | per-symbol session tests; `ISizingPolicy` tests carry the old `position_sizing_bridge` cases unchanged (ADR D17) — **done PR 2.1d**: the four bridge cases moved verbatim, and the five sizing cases came out of `unit/domain/backtesting/policies/test_margin_risk_policy.py` with their numbers and comments unchanged into `unit/modules/strategy/domain/policies/test_margin_sizing_policy.py`, the only edit being that the answer is a named `MarginAllocation` rather than a bare pair. That file keeps its other five cases, and says at the top where the missing ones went. `SizingPolicyContract` is new, and it runs against the real implementation in unit rather than against a fake in unit and the real one in integration — §10.3 rule 3's shape assumes a port with something to stand in for, and this one is arithmetic |
...
| 4 | `unit/presentation/ui/{kit,qml,components,common,state,registry}` → `support/ui_kit` and `support/charting` mirrors (about 90 files; 2 went early in PR 1.6a with `assets/` — `test_icon_loader.py` and `test_asset_validator_extension.py` → `unit/support/ui_kit/assets/` — the whole `unit/presentation/ui/kit/` tree in PR 1.6b, 4 of `unit/presentation/ui/common/` in PR 1.6c, and `unit/presentation/ui/{state,registry}` plus the sidebar, symbol-picker and log-panel files in PR 1.6d — because §9.3's mirroring rule follows the subject, not the phase number. `health_check_coordinator` and `health_status_report` moved with **no** mirror file to move: they are exercised from `screens/test_system_health_logging.py` and one integration flow, which is where a helper with no test of its own is actually proven and where those tests stay. `test_palette_is_the_only_color_source.py` did **not** move: it is a scanning guard over *both* trees for as long as both exist) | — | — | the UI-map guard |
