...
| `infrastructure/binance/exchange_session_factory` (needs `PythonBinanceClient`, which is market_data's own adapter), `application/ports/i_exchange_session_factory` (returns `IExchangeClient`, a market-data shape) | `support/binance_gateway` / `modules/market_data/contracts` | 0 — **PR 0.4**, with the module that owns the other half |
