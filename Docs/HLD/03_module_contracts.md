...
| `qml/kit`, `qml/DataTable`, `components/{sidebar,environment_banner,market_picker}` | `support/ui_kit` | 4, or earlier — `sidebar` and `environment_banner` are leaves now that `kit/` has gone; `market_picker` still waits on `qml/SelectList`, which Phase 4 deletes, and the two `qml/*` rows are arguably not worth moving at all since ADR D21 deletes them in the same phase |
