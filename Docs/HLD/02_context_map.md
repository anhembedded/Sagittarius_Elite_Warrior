...
```
   ...
   └──────────────┘                          └──────────────┘   .claim_symbol()     └──────────────┘
          ...
          ▼                                         │                                        │  business module)
   ┌──────────────┐ ────────────────────────────────┘
   │ backtesting  │  (Supporting) — no ACL: IStrategyEngine is the boundary
   └──────────────┘
...
| `backtesting` ⇄ `PaperExchange` versus `StrategyContext` | **Anticorruption Layer** inside `backtesting` | `strategy` defines its own `StrategyContext` (candles plus the current position as a neutral value object); `backtesting/adapters/` translates `PaperExchange` into it; `trading` provides one from `LivePosition`. (Round-3 correction: the earlier claim that `strategy_context.py` imports `domain/backtesting` was **false** — measured, nothing under `domain/strategies/` does. The real wrong-direction import is **`trading → backtesting`**: `domain/trading/policies/position_sizing_bridge.py:21` imports `MarginRiskPolicy` from `domain/backtesting/policies/`. That is a dependency §2.1 forbids, drawn on no diagram. **Decided — ADR D17:** sizing belongs to `strategy`; `MarginRiskPolicy` and the bridge move there in Phase 2 behind `ISizingPolicy`, which both `trading`'s callers and `backtesting` consume. **Done — PR 2.1d, and it was a split rather than the whole-file move the decision described:** only `calculate_margin_and_notional()` was sizing; `MarginRiskPolicy`'s other three methods are leverage by direction, mark-to-market and realized PnL — a paper broker's books — and moving those into `strategy` would have drawn the same arrow backwards in Phase 3, when `modules/backtesting` would have had to reach into `modules/strategy` for `calculate_realized_pnl`. So the sizing rule is `modules/strategy/domain/policies/margin_sizing_policy.py` behind `ISizingPolicy`, the rest stayed, the formula is unchanged, and `trading → backtesting` is gone from the allowlist.) **Round-4, and the ACL itself is now measured out (`EPIC-025D` §8): it is not built, and should not be.** Nothing outside `modules/strategy` constructs a `StrategyContext` — `StrategyEngine` builds it, and all three of `backtesting`'s call sites hand the engine *one fact*, `exchange.current_side`, a `PositionSide` from `trading/contracts`. `IStrategyEngine.on_tick()` **is** the anticorruption boundary, built in PR 3.1b. Building the folder would require *publishing* `StrategyContext`, whose indicator map carries `MACDValue` and `SupportResistanceValue`, and a published contract may not carry a domain type — the rule that deleted `IStrategyCatalog` in PR 2.1c. Measured: `backtesting` imports four `strategy.contracts` modules and nothing else, with zero allowlist lines in that direction. An ACL translates a foreign model that leaks in; nothing leaks in, and a layer whose whole job is `PositionSide → PositionSide` is the accidental complexity ADR D2 exists to avoid. |
...
| `PositionSizing`, `PositionSizingType` | backtesting, trading (`position_sizing_bridge`), strategy | ✅ **`core/vo/position_sizing.py` since PR 2.1d.** The row's own split held exactly as written: the value type is neutral data (a frozen dataclass, a `str` enum and range validation, no trading rule), so it moved as a move plus an import rewrite; the *rule* went to `strategy` as `ISizingPolicy`. The consumer list is wider than 2026-09-11 measured — the backtest UI, both backtest commands and `trade_once_cmd` name it too, which only strengthens the admission |
...
| `SignalAction`, `Signal`, `LiveStrategyConfig` | strategy only (plus trading through the bridge) | ✅ **`modules/strategy/contracts/` since PR 2.1b.** *"Plus trading through the bridge"* turned out to be the whole difficulty: the bridge (`order_intent_for()`, a `SignalAction →` table) was sitting in `modules/trading/domain/policies/` although **nothing in that module called it**, so moving `SignalAction` here would have made `trading` import `strategy` — which `EPIC-025C` §2 forbids outright. PR 2.1a published the `(side, reduce_only)` pair trading does own as `trading/contracts/order_intent.py` and PR 2.1b brought the table here with its two callers, so the sentence in this row now describes a one-way dependency: `strategy` reads `trading/contracts`, and `trading` names nothing of strategy's |
| `BrokerSimulationConfig`, `CommissionType` | backtesting only | ❌ → **`backtesting/contracts` since PR 3.1c**, not `domain/` as this row said. The refusal from `core/vo` held on its own measurement — one module's vocabulary, and §2.4 admits a type only with two consumers in two modules — but `domain/` was the wrong destination: the Backtest screen's configuration form builds a `BrokerSimulationConfig` and names `CommissionType` in a combo box, so the type crosses the module boundary on every run. A boundary-crossing type is published whether or not anyone declared it so (PR 1.3b's measurement), and `Currency` came with them for the same reason |
...
| `BacktestCompletedEvent`, `BacktestFailedEvent` | `backtesting` | the Backtest screen, which is **not** in the module yet | **`contracts/events/` since PR 3.1c** — this row said *internal*, and "only its own screen listens" was true and still the wrong conclusion: the screen is 74 files outside the module until Phase 4, so *its own screen* is a cross-boundary subscriber today. Once Phase 4 moves it the event becomes internal in fact; it stays published, because HLD §3.4's `IBacktestRunner` would hand these back |
