...
class BenchmarkMetrics:
    ...
    def mock_emit_region(self, *args, **kwargs): pass
    def mock_emit_info(self, *args, **kwargs): pass
    def mock_emit_markers(self, *args, **kwargs): pass
    def mock_on_error(self, *args, **kwargs): pass
...
def run_batch_approach(candles, registry, scripts_to_run):
    ...
    runner = IndicatorScriptRunner(
        registry=registry,
        emit_line=lambda *args: None, # Vô hiệu hoá emit liên tục trong vòng lặp của app
        emit_region=metrics.mock_emit_region,
        emit_info=metrics.mock_emit_info,
        emit_markers=metrics.mock_emit_markers,
        on_error=metrics.mock_on_error
...
def main():
    ...
    current_time, current_metrics = run_current_approach(candles, registry, scripts_to_run)
    ...
    if batch_time > 0:
        ...
        print(f"=> Rác luân chuyển qua RAM giảm {(current_metrics.total_data_copied - batch_metrics.total_data_copied) / current_metrics.total_data_copied * 100:.2f}%")
