...
def test_all_config_files_loaded():
    ...
    assert os.path.exists(ui_matrix_json), f"ui_matrix.json is missing at {ui_matrix_json}!"
    ...
    assert "data_management" in all_cfg, "Missing 'data_management' section from ui_matrix.json!"
    assert "LOCKED" in all_cfg["data_management"], "'data_management' section must define LOCKED mode"
