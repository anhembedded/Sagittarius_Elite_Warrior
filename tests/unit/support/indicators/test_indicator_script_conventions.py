...
def _bot_root() -> Path:
    """The repository root, found by a **landmark** rather than by counting
    directories up.

    `EPIC-025` PR 1.6g is why. This was `parents[3]`, correct while the file
    sat at `tests/unit/domain/`; the move to `tests/unit/support/indicators/`
    put it one level deeper and `parents[3]` became `tests/`, so the guard went
    looking for `tests/src/binance_bot_module.py` and raised
    `FileNotFoundError`. That is the second instance of the same class in two
    pull requests — `chart_toolbar.py`'s `parents[2]` was the first — and a hop
    count is broken by any move that changes depth, which is every move this
    epic makes. `tests/unit/architecture/test_no_root_is_found_by_counting.py`
    now fails on the pattern; this function is the shape it asks for.
    """
    for candidate in Path(__file__).resolve().parents:
        if (candidate / "pyproject.toml").is_file():
            return candidate
    raise RuntimeError("no pyproject.toml above this file — where is the repo?")
