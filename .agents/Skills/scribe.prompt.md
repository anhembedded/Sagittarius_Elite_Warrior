...
the gate at the baseline it measured, not at zero: that block lists the files
whose pre-existing errors are frozen as debt. Shrinking whatever is still on the
list today is exactly the size of your run, and it moves a live epic forward.

```bash
grep -n 'tool.mypy' -A 80 pyproject.toml     # the debt list, always current
```

Pick one excluded file, make it type-clean, remove its line. A measurable,
self-refreshing supply of work nobody has to write down for you.
...
When the debt list has nothing you can take, the second seam is documentation: a
domain entity, port or use-case handler whose docstring does not describe what it
actually promises. [`domain-truth-rule.md`](../rules/domain-truth-rule.md) cares
about exactly this — a snapshot or metric that does not say what window, fee
model or execution mode produced it is a number nobody can trust.
...
✅ **Always:** prefer a `dataclass(frozen=True)` or a value object over a raw dict
or tuple. Use precise types (`Sequence[T]`, `tuple[int, ...]`, `X | None`), not
`Any`.
...
1. **Audit** — read the debt list, pick one file, run `mypy` on it and read every
   error before deciding it is your target.
...
4. **Verify** — the gate ([README](README.md) §3). If you removed a line from the
   `mypy` block, that removal *is* the proof: the gate now checks the file for
   real.
5. **Present** — `refactor(types): <subject>` or `docs(<scope>): <subject>` per
   [`commit-rule.md`](../rules/commit-rule.md). Name the file you took off the
   debt list.
