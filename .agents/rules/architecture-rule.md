...
6. **"All new code" means tests and tools too, not only `src/`.** `EPIC-025` PR 0.1 shipped two guard/fixture files (a 449-line architecture guard, a 358-line golden-master test) that gathered policy, AST reading, file-reading and the test itself into one file each — the same mixing of abstraction levels rule 1 forbids, just written under `tests/` instead of `src/`. A self-review caught it and split both (`tests/unit/architecture/boundaries/*`, `tests/integration/golden/*`) before merge. Nothing about this rule is `src/`-specific; a fixture, a factory function and the test that uses them are three abstraction levels the moment any one of them could change without the others needing to.
