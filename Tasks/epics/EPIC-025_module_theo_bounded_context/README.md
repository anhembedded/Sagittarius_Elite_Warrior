...
| **1.4** | Trading and Dev Board become surfaces on a nested `QMainWindow`: contributed panels / dialogs / actions, perspective per user. **This is where the duplicated presentation members go to their target**, and where the `ui/common` items used only by these two screens move. **Four slices**, for the same reason 1.3c had five — the mechanism can fail without a screen, and each screen can fail without the other: **1.4a** ✅ the surface host itself (`shell/workbench_surface.py` and `shell/surface_building.py`, so `IPlaceHost` finally has an implementation — no screen moved); **1.4b** Trading's positions and open-orders tables off QML-in-`QQuickWidget` onto `QTableView` panels the module contributes; **1.4c** Dev Board's panels and its order dialog, plus the first `dev_probe`; **1.4d** the `ui/common` items only these two screens use, and the duplication number restated with the command that measures it (the epic's stale "59" measures 34 at presenter level and 79 at package level today) | 0 | **high** — the largest UI change in the epic |
