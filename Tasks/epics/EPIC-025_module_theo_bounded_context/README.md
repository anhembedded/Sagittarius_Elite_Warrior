...
| **1.5** | The **Welcome** surface (ADR D13, D14) as the default route, and the `dev.mode` toggle with Restart. Two steps: **1.5a** ✅ the surface itself — the first screen the shell owns, contributed as a `ScreenContribution` like every other, default route, and **Start** as an intent on the bus so *Main* decides where it goes; **1.5b** the developer-mode switch with Restart, and the Dev Board's *screen* gated on `dev.mode` (its surface already is). The first `dev_probe` moved to 1.4c-4, where the contribution path it proves was built | 0 | medium |
