...
| **1.1b** | `IMarketStream`, and `interactive_shell.py` moves into `shell/`, retiring the two CLI lines. Split out of 1.1 **after** 1.1a was written, not planned that way: the consumer move alone touched 28 files and repaired 65 tests that a stubbed dispatcher had been keeping green, and a second port in the same diff would have doubled a review without adding a way for it to fail | 6 | low |
