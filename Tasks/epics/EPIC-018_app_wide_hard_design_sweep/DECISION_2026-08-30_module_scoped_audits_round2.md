...
| D-app-2 | 5 Query class còn `interval: str` + convert muộn `TimeFrame(query.interval)` sâu trong `execute()`: `GetDatabaseGapsQuery`, `GetDatabaseStatusQuery`, `GetHistoricalKlinesQuery`, `ScanAllDatabasesQuery.intervals: list[str]`, `GetBacktestRangeCoverageQuery` — đúng hình dạng `EPIC-018A` đã sửa cho `AuditDatabaseIntegrityQuery`, `EPIC-018A`'s task doc tự ghi nhận đây là phạm vi khác chưa làm | ✅ Đúng cho 4/5; `ScanAllDatabasesQuery.intervals` **sai** khi áp cùng cách sửa — verify lúc code: đây là `list`, không phải giá trị đơn, và `test_invalid_interval_is_skipped_gracefully` xác nhận "bỏ qua phần tử interval sai, không phá cả batch" là hành vi có chủ đích, không phải bug | 🔵 Sửa 4/5 (`GetDatabaseGapsQuery`, `GetDatabaseStatusQuery`, `GetHistoricalKlinesQuery`, `GetBacktestRangeCoverageQuery`) — đổi field sang `TimeFrame`, validate fail-fast lúc dựng. ❌ `ScanAllDatabasesQuery.intervals` **giữ nguyên** `list[str]` — ép validate cả list lúc dựng sẽ đổi hành vi partial-degradation thành all-or-nothing, không phải "fail fast tốt hơn" |
...
| F6 | `binance_bot_module.py:212-214` — `os.path.join(os.getcwd(), ...)` viết thẳng ở composition root, trong khi `main.py:89-91` đã có tiền lệ `PathUtils.get_relative_path(...)` cho đúng việc này | ❌ **Sai** — verify lúc code: `PathUtils.get_relative_path(base_file, *paths)` resolve tương đối `__file__`, không phải CWD. Đổi sẽ đổi hành vi thật (thư mục DB mặc định chuyển từ CWD-relative sang file-location-relative), không phải refactor thuần | ❌ **Giữ nguyên** — 1 dòng `os.path.join` đơn giản, không phải "deep os.path joins" mà rule nhắm tới; đề xuất gốc dựa trên tiền lệ giải quyết bài toán khác (tìm config cạnh source, không phải resolve theo CWD) |
