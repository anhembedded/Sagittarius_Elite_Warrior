...
| **[EPIC-006](EPIC-006_drop_qml/README.md)** | Bỏ hẳn QML, thuần QtWidgets | 🟡 Đang làm (5/6 task con xong) — đã merge vào `master-warrior` (`f4076a7`); còn `EPIC-006F` dỡ kit QML bên Engine |
...
| **[EPIC-008](EPIC-008_chuan_hoa_luong_event/README.md)** | **Chuẩn hoá luồng sự kiện** — Shared Kernel + port, `BaseEvent` kế thừa được thật, `EventRegistry` + catalog sinh tự động, 3 Feed thay 48 signal cầu nối | 🟡 Đang làm (4/8) — `008A`–`008D` ghi xong ở repo **Engine** nhưng **chưa xác minh được từ laptop** (có thể còn ở PC chưa push); tiếp theo `008E` |
