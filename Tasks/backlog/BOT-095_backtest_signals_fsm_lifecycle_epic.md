...
| 🔹 **[BOT-095C](BOT-095C_backtest_cancellation_and_stop_button.md)** | **Nút Hủy / Dừng Backtest & Tiến độ Tính toán Realtime (`CancellationToken` & ETA)** | **P1 (UX Safety)** | Nút Cancel khi đang RUNNING, tích hợp CancellationToken ngắt luồng an toàn, thanh Progress Bar % + thời gian còn lại ETA. |
| 🔹 **[BOT-095D](BOT-095D_backtest_timeframe_change_and_data_probe.md)** | **1-Click Auto-Sync & Run, Date Range Gap Check & Live Preview** | **P1 (Data Flow)** | Kiểm tra độ bao phủ nến (Range Gap Check), 1-Click Auto-Sync & Run kế thừa Progress Bar từ Data Management, preview nến mới khi đổi Timeframe. |
...
| 🔹 **[BOT-095F](BOT-095F_backtest_dynamic_indicator_toggle.md)** | **Toggle Chỉ báo Tham chiếu Động trên Biểu đồ sau Backtest** | **P2 (Visualization)** | Bật/tắt chỉ báo tham chiếu (RSI, MACD, EMA) trên biểu đồ mà không cần chạy lại backtest. |
| 🔹 **[BOT-095G](BOT-095G_backtest_session_run_history_cache.md)** | **Bộ nhớ đệm Lịch sử Lần chạy (Session Run History Cache)** | **P2 (UX Power)** | Lưu 5 lần chạy gần nhất, dropdown xem lại tức thì ($0\text{ms}$), phục hồi hoàn hảo Charts, Metrics và Trade Logs để trader so sánh đa khung/đa chiến lược. |
...
3. **Visual & UX Verification**:
   ...
   - Bấm Hủy trả lại quyền điều khiển UI trong $< 200\text{ms}$.
