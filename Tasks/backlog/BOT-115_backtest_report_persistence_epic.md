...
| ✅ **[`BOT-115D`](../completed/BOT-115D_backtest_report_side_by_side_comparison.md)** | **So sánh 2 báo cáo cạnh nhau** | 🟡 `M` | **Xong (24/09)**: modal "Compare reports", Column A = kết quả đang hiển thị, Column B = tải từ file. Bảng diff config (tái dùng `compute_diff_summary()`), metrics side-by-side + cột chênh lệch tô màu đúng chiều, 2 đường vốn chuẩn hoá chồng lên nhau. |
