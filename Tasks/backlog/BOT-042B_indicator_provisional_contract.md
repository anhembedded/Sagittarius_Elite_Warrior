...
- [ ] `src/domain/indicators/i_indicator.py`: thêm `peek_provisional` vào
      `Protocol`.
- [ ] `EMA.peek_provisional()`: đúng công thức `update()` nhánh sau warm-up,
      dùng biến cục bộ thay vì gán `self._ema`. Trong lúc `self._ema is
      None` (đang warm-up), trả `None` — **không** đụng `self._seed_values`.
- [ ] `RSI.peek_provisional()`: tương tự — không gán `self._avg_gain`/
      `self._avg_loss`/`self._previous_close`/`self._gain_sum`/`self._loss_sum`.
- [ ] `WMA.peek_provisional()`: dựng view tạm từ `self._values` (deque) +
...
- [ ] `MACD.peek_provisional()`: gọi `peek_provisional()` của `_fast_ema`/
...
- [ ] Với từng indicator: gọi `peek_provisional()` N lần bất kỳ (N ngẫu
...
- [ ] Với từng indicator: `peek_provisional(value)` gọi ngay sau khi
      `update()` vừa chốt cùng `value` đó phải trả **kết quả giống hệt**
      giá trị `update()` vừa trả (tính nhất quán công thức).
- [ ] Toàn bộ test hiện có của `EMA`/`RSI`/`MACD`/`WMA` phải xanh **không
