...
| Execution Trigger Rules — **On historical/realtime bar tick** | ❌ Cần `BOT-042` | `StrategyEngine`/`IIndicator` hiện chỉ nhận 1 giá trị đóng nến/lần gọi (`update(value: float)`) — không có khái niệm "giữa nến". User tự làm phần nạp dữ liệu tick 1s; `BOT-042` là phần engine cần để *dùng* được dữ liệu đó. |
| Execution Trigger Rules — **On order filled** | ❓ Chưa rõ nghĩa trong ngữ cảnh backtest | "Trigger khi lệnh khớp" nghe giống 1 kiểu re-entry/pyramiding logic hơn là 1 chế độ tính chỉ báo — cần user giải thích thêm ý định trước khi viết action item. |
...
| Task | Mô tả ngắn |
| :--- | :--- |
| [`BOT-044`](BOT-044_param_schema_core.md) | **Param Schema Core** — value object + API `input_int/float/bool/string()` trên `BaseIndicatorScript`, cơ chế 2 pha (khai báo → đọc schema → tạo lại với params). ⚠️ Đảo ngược `BOT-032` §9.1. |
