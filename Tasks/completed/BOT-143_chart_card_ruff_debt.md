...
Verification: `ruff check`/`ruff format --check src tests tools scripts` clean; `tests/unit/support/charting` (199) and `tests/unit/architecture` (425) passed against the fixed tree; full `tests/unit` run — see the PR for the final count. No production behavior changed — both constants replace a literal with the identical value, and `ruff format` only reflows already-equivalent expressions.
