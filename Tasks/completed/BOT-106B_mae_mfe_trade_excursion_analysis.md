...
- **Presentation: ngoài phạm vi.** `BackTestTradeLogs.qml` không còn tồn tại — QML đã bị gỡ khỏi codebase này trước `EPIC-025` (đã xác nhận ở phiên trước). Domain-only là điểm dừng đúng, cùng khuôn với `BOT-106C`. Việc trình bày được ghi thành task riêng có theo dõi —
...
- **Giới hạn thật lúc viết task này, đã đóng cùng ngày — [`BUG-133`](../bug_report/completed/BUG-133_tick_backtest_never_checks_intrabar_stops.md) ✅:** `RunHistoricalTickBacktestCommandHandler` (`BOT-076`, Historical Tick Backtest) **không bao giờ gọi** `check_intrabar_stops()` — không riêng MAE/MFE, mà cả SL/TP (`BOT-041`) và thanh lý (`BOT-049`) cũng vô hiệu ở chế độ này từ trước khi task này tồn tại. MAE/MFE lúc đó **chỉ hoạt động ở chế độ Static Backtest**; không mở rộng phạm vi để tự sửa `BUG-133` ở đây (`ONBOARDING.md` §7), đã ghi hồ sơ riêng — hồ sơ đó đã đóng cùng ngày (22/09), MAE/MFE nay hoạt động ở cả hai chế độ, không cần sửa gì thêm ở task này.
