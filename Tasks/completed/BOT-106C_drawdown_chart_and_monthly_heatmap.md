...
- **Presentation: ngoài phạm vi, không phải bỏ sót.** `MonthlyReturnsHeatmap.qml`/`DrawdownUnderwaterCard.qml` không dựng được — QML đã bị gỡ khỏi codebase này trước `EPIC-025`. Domain-only là điểm dừng đúng, cùng khuôn với `BOT-106B`; UI thật (PySide6, theo `ui-presentation-rule.md`) là việc của một task riêng khi màn Backtest cần hiển thị hai biểu đồ này.
