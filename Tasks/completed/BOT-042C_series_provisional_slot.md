...
- [x] Thêm sentinel riêng (không phải `None`, vì `None` là giá trị hợp lệ
      cho 1 bar "chưa có giá trị") để phân biệt "chưa có tick nào trong bar
      này" với "tick báo giá trị `None` có chủ đích".
- [x] `poke_provisional(value: float | None) -> float | None`: ghi đè ô tạm.
      Gọi nhiều lần trong cùng 1 bar chỉ ghi đè, **không** đẩy slot mới vào
      `self._values`.
- [x] `push(value)`: xoá ô tạm (reset về sentinel) rồi mới ghi vĩnh viễn như
      hiện tại — **hành vi ghi vĩnh viễn giữ nguyên 100%**.
- [x] `__getitem__(offset)`: khi có ô tạm active, `offset=0` đọc ô tạm,
...
- [x] `current`/`previous` property: tự động đúng vì chỉ gọi `self[0]`/
      `self[1]` — không cần sửa riêng.

## 3. Guard test bắt buộc (viết trước khi coi là xong)

- [x] Gọi `poke_provisional()` nhiều lần với giá trị khác nhau trong "1 bar
...
- [x] `crossed_above`/`crossed_below` với 1 series có provisional active:
      ...
      nhất của cả `BOT-042`.
- [x] Toàn bộ test hiện có của `Series`/`crossed_above`/`crossed_below`/
...
- [x] **Test mới, riêng cho ca "tick đầu tiên của cả run"**: gọi
...
> ✅ **Hoàn thành (2026-08-19)**: sentinel `_NO_PROVISIONAL` (module-level
> `object()`, phân biệt với `None`). 5 test mới (bao gồm cold-start và
> "crossed_above không báo giả trong cùng 1 bar") + 15 test cũ giữ nguyên =
> 20/20 pass. Toàn bộ `tests/unit/domain/` (226 test, cả indicators lẫn
> scripting) xanh. `ruff check`/`format` sạch.
