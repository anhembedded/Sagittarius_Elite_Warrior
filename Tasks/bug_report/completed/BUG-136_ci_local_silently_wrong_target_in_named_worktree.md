...
`tests/unit/scripts/test_ci_local_checkout_name_guard.py` (new, +3) — invokes the real `scripts/ci-local.ps1` (not a reimplementation) inside throwaway directory trees and asserts on the script's own machine-readable `===CI_LOCAL_RESULT===` block: `review-worktree` (a completely different name) and `sagittarius_elite_warrior` (a **case-only** mismatch — the exact scenario `install-rule.md` §2b names as the standard real-world trigger, a reviewer's default lowercase clone) must both report `RESULT: FAIL` / `FAILED_STEPS: Checkout Name`; `Sagittarius_Elite_Warrior` must be unaffected (`RESULT: PASS` / `FAILED_STEPS: none`). Mutation-verified twice: (1) reverting `$checkoutNameValid` to a hardcoded `$true` sent the mismatched-name test red for the right reason; (2) reverting the comparison operator from `-ceq` back to PowerShell's default case-insensitive `-eq` — the defect an independent reviewer of PR #269 caught, since `-eq` silently accepts a case-only mismatch — sent the case-only test red for the right reason (`RESULT` was `PASS`, expected `FAIL`). Both restored after confirming.

## Verification

`ruff check`/`ruff format --check` clean on the new test file. `tests/unit/scripts/test_ci_local_checkout_name_guard.py`: 3 passed. `tests/unit/architecture`: 440 passed, no regressions. `python3 scripts/check_skill_prompt_references.py`: OK. Manually reproduced both before (silent wrong-tree PASS, matching the original report) and after (loud `RESULT: FAIL`) in a throwaway `/tmp` directory tree mirroring the bug's own reproduction steps, and confirmed a normal, correctly-named run of this repository's own real gate (`ci-local.ps1 -SkipTests`) is unaffected (`RESULT: PASS`).
