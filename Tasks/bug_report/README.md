...
| :--- | :--- | :---: | :---: | :--- |
| **[BUG-052](incomplete/BUG-052_process_does_not_exit_after_graceful_shutdown.md)** | Thoát app: shutdown chạy hết, log `App stopped.` nhưng tiến trình không return | 🟠 P1 | 2026-08-26 | **Cập nhật 2026-08-26:** cơ chế đã chứng minh bằng đo — chỉ **non-daemon thread** giữ được tiến trình; `concurrent.futures` đăng ký `_python_exit` **join mọi worker bất kể `wait=False`**, và join đó xảy ra *sau khi logging đã dừng* nên treo ở đó **im lặng theo thiết kế**. Đã **sửa một suy luận sai** trong hồ sơ: `ThreadManagerExtension` dừng trong 4ms **không** chứng minh hết job (vì `wait=False` luôn trả về ngay). Tái hiện được trong đường shutdown thật (`scripts/bug052_shutdown_thread_probe.py`); baseline sạch. **Chưa biết thread cụ thể** — backtest đã xong trước đó 73 phút. Đã thêm chẩn đoán vào `teardown()` để lần tái hiện sau tự nêu tên thread + stack. |
