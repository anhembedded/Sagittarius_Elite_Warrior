...
| P6 | **Fix the mechanism, general over local** — cost is never the reason to prefer the local fix. | Deming (1986): most defects come from the system, not the individual act; root-cause analysis (Ohno's "five whys", Toyota). | `fix-bug-rule` §2; review row E10 |
