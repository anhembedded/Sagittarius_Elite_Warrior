...
|---|---|---|---|---|
| ~~TC-ASY-01~~ | ✅ | Click "Load History" 2 lần liên tiếp trong <200ms (trước khi lần 1 xong) | ~~`load_history_button` không bị disable ở IDLE matrix~~ | **FIXED (BOT-027, cơ chế thay bằng BOT-069)** — `StreamLifecycleController._on_load_history` giờ gọi `ExclusiveAction.try_start("load_history")` ở đầu hàm và trả về ngay nếu đã có key nào đang chạy (thay cho cờ `historyLoading` viết tay trước đây), chặn hoàn toàn lần click chồng thay vì cho 2 background task chạy song song đọc chung 1 snapshot indicator. Verify bằng test thật (không mock `submit`, dùng `ThreadPoolExecutor` thật): `test_concurrent_load_history_clicks_keep_indicator_series_correct`, `test_load_history_button_is_disabled_during_background_load` (`tests/integration/presentation/ui/test_dev_board_async_race_conditions.py`). |
...
| ~~TC-ASY-03~~ | ✅ | Click "Load History" rồi trong lúc đang chạy, click "Start Live" | ~~2 workflow này không loại trừ lẫn nhau~~ | **FIXED (BOT-027, cơ chế thay bằng BOT-069)** — `_on_load_history` và `_on_start_stream` giờ dùng **chung 1 instance** `ExclusiveAction` (`self._stream_actions`), nên key nào đang chạy (`load_history` hay `start_stream`) tự động khoá key còn lại — chặn chồng chéo theo cả 2 chiều giữa 2 entry point bằng kết cấu dữ liệu chứ không phải 2 đoạn code viết tay phải nhớ đồng bộ với nhau. FSM `!= IDLE` check của `_on_start_stream` vẫn giữ nguyên riêng (chặn "start khi đã LIVE/ERROR", khác hẳn "start khi đang có tác vụ chạy"). Vẫn còn phần "SQLite WAL lock khi Sync ghi đúng lúc Load đọc" chưa có test riêng — không còn tái hiện được nữa vì giờ 2 workflow không thể chạy đồng thời. **Lần đầu có test thật cho đúng case này** (trước đó chỉ là suy luận thiết kế, chưa test): `test_starting_the_live_stream_while_a_history_load_is_in_flight_is_rejected` (`tests/integration/presentation/ui/test_dev_board_async_race_conditions.py`). |
| ~~TC-ASY-04~~ | ✅ | 4 click "Load History" liên tục thật nhanh (chạm max_workers=4) | ~~Cả 4 worker thread của pool đều bận với 4 lần `_run_load_history` chồng nhau~~ | **FIXED (BOT-027, cơ chế thay bằng BOT-069)** — cùng guard `ExclusiveAction` ở trên: chỉ click đầu tiên submit task, 3 lần còn lại bị chặn ngay trên main thread nên không còn 4 worker nào chạy chồng để so sánh. Verify: `test_four_rapid_load_history_clicks_keep_indicator_series_correct`. |
...
| TC-GAP-04 | ~~`strategy_dropdown` (Manual/SMA Crossover)~~ | ✅ **FIXED (`EPIC-023C`), không phải bằng `BOT-039`** — cả control lẫn `DevBoardPanel.qml` chính nó không còn tồn tại (QML gỡ sạch ở `EPIC-025`). Thay bằng thẻ "Nạp chiến lược" thật (`_build_strategy_card()`), nối `StrategyArmingCoordinator`, mirror `TradingView` — khác hẳn kế hoạch toggle-list nhiều strategy cùng lúc mà `BOT-039` từng đề xuất (đã huỷ, xem hồ sơ huỷ) | `dev_board_panel.py::_build_strategy_card`, `strategy_arming_coordinator.py` |
...
- **✅ FIXED (BOT-027, cơ chế thay bằng BOT-069)**: TC-ASY-01, TC-ASY-03, TC-ASY-04 — ban đầu
  bằng guard `historyLoading`/FSM viết tay, sau đó BOT-069 thay bằng `ExclusiveAction`
  (`sagittarius_engine/runtime/tasks/exclusive_action.py`), 1 instance dùng chung giữa
  `_on_load_history`/`_on_start_stream` (`stream_lifecycle_controller.py`) — chặn mọi lần click
  chồng chéo ngay trên main thread, cả trong nội bộ 1 nút lẫn xuyên 2 entry point, bằng đúng 1
  kết cấu dữ liệu thay vì 2 đoạn code phải nhớ đồng bộ tay. Đây chính là "Fix đơn giản, rủi ro
  thấp" từng đề xuất bên dưới, đã triển khai rồi được nâng cấp thành cơ chế tái dùng được.
