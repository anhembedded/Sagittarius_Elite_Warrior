...
| **[EPIC-003](epics/EPIC-003_presenter_and_god_file_decomposition/README.md)** | Phân rã Presenter/File quá tải (Coordinator Pattern, Domain Policy, QML) | 🟡 Đang làm (4/7 xong, 1 huỷ — `003D`; còn `003F`, `003G`) |
...
| **[EPIC-015](epics/EPIC-015_qml_tung_widget_khong_chuyen_chart/README.md)** | **QML từng widget, chart ở lại QtWidgets** — QML lồng được vào QtWidgets, chiều ngược lại thì không, nên chart pyqtgraph (4.253 dòng) và shell ở lại widget, mọi thứ khác chuyển dần: modal → Settings → Data Management → Dev Board → Backtest. Widget ở DIR riêng, ViewModel của widget test riêng không cần GUI (đã đo) | 🟡 **Đang làm** — 8 widget/kit mới xây đứng riêng (29-30/08). **Phase 1+2+3 xong toàn bộ** (30/08): `SymbolPicker`/`TimeRangePicker`/`TimeframePicker` (modal)/`KlineInspectorTable` (Phase 1) + `DatabaseStatusTable`/`ProgressBanner` ở Data Management (Phase 2) + `MetricsDetailPanel` thay `ExtendedMetricsDialog`/`StatGrid` ở Backtest (Phase 3, 3 ngưỡng verdict Sharpe/Sortino/Calmar tự chế đã được duyệt shipping nguyên trạng). **Phase 4 code+test xong** (`TimeframeToolbar`/`StatusPill`/`StatCard`+`ProgressBanner` cạnh chart thật) — **chờ user tự chạy app xác nhận chart không giật/rớt khung hình** trước khi tính là xong hẳn — 2026-08-30 |
| **[EPIC-016](epics/EPIC-016_screen_registry_pattern/README.md)** | **Screen Registry Pattern** — `MainWindow` hard-code toàn bộ nav + router setup; `ISidebar`/`AbstractScreenModule`/`IScreenRegistry` tách nó ra, không tạo type song song với `ITab`/`SidebarSection` có sẵn. ADR *Approved*, D4 (xác nhận engine thật) còn Open, chặn `016B` | 🟡 Đang làm (0/4 task con) |
| **[EPIC-017](epics/EPIC-017_presentation_hard_design_round2/README.md)** | **Presentation Hard-Design Round 2** — verify độc lập report ngoài: Settings hard-code field 3 màn khác, dùng `TimeFrame` Enum đã có sẵn thay literal `"1m"`; God Presenters route sang `EPIC-003` (`003G`); 2 đề xuất bị từ chối (auto-discovery, bỏ Junction hack) | 🟡 Đang làm (0/2 task con) |
