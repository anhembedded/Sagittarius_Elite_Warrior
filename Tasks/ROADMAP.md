...
> Khảo sát để lập epic này mở thêm 2 bug: [`BUG-078`](bug_report/incomplete/BUG-078_settings_api_credentials_never_reach_the_exchange_client.md)
> (key nhập ở Settings không bao giờ tới client) và [`BUG-079`](bug_report/incomplete/BUG-079_binance_endpoint_config_keys_are_dead.md)
...
| **[EPIC-021](epics/EPIC-021_ket_noi_binance_futures_testnet/README.md)** | **Kết nối Binance USD-M Futures Testnet & đường đi lệnh thật** — app hiện **không có khái niệm môi trường sàn**: DI dựng `PythonBinanceClient` không tham số (`binance_bot_module.py:231`) nên luôn mainnet ẩn danh, key user nhập ở Settings không bao giờ tới client (`BUG-078`), 2 key endpoint trong config là config chết (`BUG-079`), và parser metadata (step/tick/minNotional) chỉ được `tests/` gọi. Chọn **Futures** thay vì Spot vì mô hình khớp lệnh của repo đã là futures (SHORT + leverage trong `PaperExchange`); Settings có **2 lựa chọn độc lập** (nguồn dữ liệu vs nơi đặt lệnh); `TradingVenue` **không có member MAINNET** — rào an toàn bằng type, không bằng cấu hình | 🔴 Chưa bắt đầu (0/11 task con — thêm màn hình **Giao dịch** mới; 4 mốc đầu chạy headless bằng CLI) |
...
- [x] **`BUG-077`**: [Chiến lược "Long Term Trend Zone" render nền xu hướng thành dải sọc gần đen trong đoạn thị trường đi ngang — `compute_strategy_trend_zones()` không có ngưỡng tối thiểu, mỗi lần `classify_trend_zone()` đảo chiều (gần mỗi nến khi giá dao động quanh EMA) tạo 1 span riêng, hàng chục span 1-nến xanh/đỏ xen kẽ đọc như khối đen. Tái hiện qua đúng code path production, loại trừ giả thuyết marker Buy/Sell Flags. Sửa: thêm `_MIN_ZONE_BARS = 3`, bỏ hẳn zone dưới ngưỡng](bug_report/completed/BUG-077_choppy_trend_zone_flips_render_as_striped_black_band.md)
