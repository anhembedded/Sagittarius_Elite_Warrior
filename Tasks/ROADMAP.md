...
| **P1** | **[BOT-036](backlog/BOT-036_indicator_feed_batching.md)** | **Gộp tín hiệu (Batching) khi replay lịch sử cho indicator script** | `BOT-032` ✅, `BOT-035` ✅ | `IndicatorScriptRunner.feed_all()` phát Qt signal mỗi nến × mỗi script × 4 kênh, mảng dài dần mỗi lần → chi phí truyền tải O(N²), nghẽn event loop khi load nhiều nến + nhiều chỉ báo. Sửa: tính im lặng (`feed(..., emit=False)`) rồi phát đúng 1 lần/kênh ở cuối; nến live giữ nguyên emit ngay. Hưởng lợi nhiều nhất là đường load-more-on-scroll (`_on_history_prepended`) — nó replay toàn bộ lịch sử phình dần **trên main thread**, mỗi emit là 1 lần pyqtgraph re-render đồng bộ. Chỉ đụng 1 file production + 1 file test; **1 test có sẵn sẽ vỡ, phải viết lại (không xoá)** — xem §6.1. |
