...
- [ ] **BOT-032**: [Custom Indicator Scripts (kiểu Pine Script, thuần Python)](in_progress/BOT-032_custom_indicator_scripts.md) — *Tự viết indicator bằng 1 class Python thường (`setup()` khai báo, `execute()` gọi `self.plot()`/`self.mark()`/`self.shade()`/`self.info()`), không tạo DSL mới. **Phase 0-2 xong**: `BaseIndicatorScript` (+ `shade()`/`info()`/`streak()`, `PlottedRegion`/`InfoField`) + `IndicatorScriptRegistry` + `domain/scripting/` (Series index kiểu Pine `a[1]`, `crossed_above/below`, cắt giá, cắt mức hằng, so sánh None-safe, `Streak`) + `WMA` + 4 script mẫu (`ema_ribbon`/`macd_full`/`ema_cross`/`dev_showcase` — 15 kỹ thuật) + `IndicatorScriptRunner`/`ScriptRegionTracker` (Presenter tính + vẽ line/background-tint/status-panel lên `ChartCard` thật, kể cả 2 signal `ui_script_region_signal`/`ui_script_info_signal`) + guard test. 354 unit test pass, screenshot offscreen xác nhận tint + panel render. Còn Phase 3 (UI bật/tắt), Phase 4 (**marker/màu-động/fill — giao nhau với BOT-026**), Phase 5 (docs), Phase 6 (chuyển RSI/EMA/MACD thành script). Task file có design + rule chi tiết cho model khác implement tiếp.*
