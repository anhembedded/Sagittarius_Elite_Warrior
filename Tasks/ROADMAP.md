...
| **[EPIC-015](epics/EPIC-015_qml_tung_widget_khong_chuyen_chart/README.md)** | **QML từng widget, chart ở lại QtWidgets** — QML lồng được vào QtWidgets, chiều ngược lại thì không, nên chart pyqtgraph (4.253 dòng) và shell ở lại widget, mọi thứ khác chuyển dần: modal → Settings → Data Management → Dev Board → Backtest. Widget ở DIR riêng, ViewModel của widget test riêng không cần GUI (đã đo) | 🟡 **Đang làm** — 8 widget/kit mới xây đứng riêng (29-30/08). **Phase 1+2 xong toàn bộ** (30/08): `SymbolPicker`/`TimeRangePicker`/`TimeframePicker` (modal)/`KlineInspectorTable` (Phase 1) + `DatabaseStatusTable`/`ProgressBanner` ở Data Management (Phase 2, lần đầu nhúng QML thẳng vào layout màn hình không qua dialog). Còn Phase 3 (`MetricsDetailPanel` — cần duyệt ngưỡng Sharpe/Sortino/Calmar tự chế) và Phase 4 (QML cạnh chart thật — rủi ro cao nhất) — 2026-08-30 |
