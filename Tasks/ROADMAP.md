...
| **P2** | **[BOT-033](backlog/BOT-033_dev_board_autostart_and_data_lifecycle.md)** | **Dev Board — Auto-start Live, Render-window tách Fetch-amount, Timeframe thật** | `BOT-032` (Phase 6, đang chốt) | Đóng 3 gap đã biết (TC-GAP-03/06/08): (1) nối thật `ChartToolbar.sig_timeframe_changed`, bỏ `cboTimeframe` cosmetic trùng lặp; (2) tách "75 nến render" khỏi "bao nhiêu nến cần tải" (`min_warmup_bars` mới trên `BaseIndicatorScript`); (3) auto-start Live Stream khi mở Dev Board, fallback sang Load History nếu không có `MarketTickEvent` nào trong ~2s (đã verify không cần thêm event-timeout vào `sagittarius_engine` — dùng `QTimer` + `MarketTickEvent` có sẵn). Toàn bộ câu hỏi UX đã được user chốt (safety floor đọc từ config mặc định 75, giữ nút Load Hist, fallback không tự retry, cho đổi timeframe khi đang Live) — sẵn sàng implement, chưa bắt đầu. |
