...
| 🟡 | **[Epic EPIC-002](epics/EPIC-002_static_type_checking_in_local_ci/README.md)** | **Kiểm tra kiểu tĩnh (`mypy`) trong CI cục bộ** | 🟢 **`S (Fast)`** | — | 3/4 task con xong (21/08) — cổng `mypy` đã sống trong `ci-local.ps1 -Full`, chi tiết ở file riêng của epic (`Tasks/epics/README.md`). |
| 🟡 | **[Epic EPIC-003](epics/EPIC-003_presenter_and_god_file_decomposition/README.md)** | **Phân rã Presenter/File quá tải (Coordinator Pattern, Domain Policy, QML component split)** | 🔴 **`L (Thinking)`** | `EPIC-002` (mypy gate bảo vệ trong lúc refactor) | 1/6 task con xong (21/08) — chi tiết ở file riêng của epic (`Tasks/epics/README.md`). |
| 🟡 | **[Epic EPIC-004](epics/EPIC-004_static_security_and_quality_analysis/README.md)** | **Static security & quality analysis gate (Bandit + magic-number + code-smell qua Ruff)** | 🟢 **`S (Fast)`** | — | 1/4 task con xong (22/08) — baseline đo xong (`ruff --select S,PLR2004,B,SIM,ERA,N`), ~48 lỗi thật cần gate, phần còn lại là nhiễu hệ thống (assert-trong-test, Qt-override-camelCase). Chi tiết ở file riêng của epic (`Tasks/epics/README.md`). |
