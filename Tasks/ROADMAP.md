...
| **P2** | **[BOT-034](backlog/BOT-034_dev_board_autostart_and_data_lifecycle.md)** | **Dev Board — Auto-start Live, Render-window tách Fetch-amount, Timeframe thật** | `BOT-032` ✅ | Đóng 3 gap đã biết (TC-GAP-03/06/08). **§6 (timeframe/`ChartToolbar`) đã xong** — trùng số ban đầu với `BOT-033` (đã đổi tên, xem cảnh báo đầu file). Còn: (2) tách "75 nến render" khỏi "bao nhiêu nến cần tải" (`min_warmup_bars` mới trên `BaseIndicatorScript`); (3) auto-start Live Stream khi mở Dev Board, fallback sang Load History nếu không có `MarketTickEvent` nào trong ~2s (đã verify không cần thêm event-timeout vào `sagittarius_engine` — dùng `QTimer` + `MarketTickEvent` có sẵn). Toàn bộ câu hỏi UX đã được user chốt — sẵn sàng implement. |
