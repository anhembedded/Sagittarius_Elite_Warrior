...
- [x] **`BUG-078`**: [Chiến lược "Long Term Trend Zone" render nền xu hướng thành dải sọc gần đen trong đoạn thị trường đi ngang — `compute_strategy_trend_zones()` không có ngưỡng tối thiểu, mỗi lần `classify_trend_zone()` đảo chiều (gần mỗi nến khi giá dao động quanh EMA) tạo 1 span riêng, hàng chục span 1-nến xanh/đỏ xen kẽ đọc như khối đen. Tái hiện qua đúng code path production, loại trừ giả thuyết marker Buy/Sell Flags. Sửa: thêm `_MIN_ZONE_BARS = 3`, bỏ hẳn zone dưới ngưỡng (đổi số từ BUG-077 do trùng với bug khác cùng số)](bug_report/completed/BUG-078_choppy_trend_zone_flips_render_as_striped_black_band.md)
