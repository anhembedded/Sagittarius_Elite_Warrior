...
| `rules/fix-bug-rule.md` | rule | every session | Root cause first, never a hotfix, log evidence, regression test before the fix at the right tier, kept forever, a report, and a case study when the gate was green. |
...
| `templates/bug-report.md` | template | on demand, copied | The format of a bug report under Tasks/bug_report/incomplete/ (fix-bug-rule.md §7). Copy it, fill every brace, delete this front matter. |
