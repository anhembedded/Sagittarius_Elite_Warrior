...
| `rules/domain-truth-rule.md` | rule | `src/modules/*/domain/**/*.py`, `src/modules/*/application/**/*.py` | The system never lies about what it did — real coverage, real exchange filters, immutable snapshots, distinct trading facts, a UI that promises only what the engine delivers. |
...
| `rules/code/errors.md` | rule | `src/**/*.py`, `scripts/**/*.py` | Error handling, failing correctly, representing invalid states, and explicit dependencies. |
| `rules/code/naming.md` | rule | `src/**/*.py`, `scripts/**/*.py` | Naming rules revealing intent and preserving domain contract clarity. |
| `rules/code/quality.md` | rule | `src/**/*.py`, `scripts/**/*.py` | Typing boundaries, top-level imports only, single-scope FSM cohesion, schemas, and suppression rules. |
...
| `skills/create-epic/SKILL.md` | skill | `/create-epic`, or Claude from its description | Scaffold, structure, and track an Epic and its child sub-tasks under Tasks/epics/EPIC-{nnn}_{slug}/. Use when planning, creating, or breaking down an epic into executable tasks. |
...
| `templates/tracking.md` | template | on demand, copied | The format of an epic's TRACKING.md under Tasks/epics/EPIC-nnn_slug/. Copy it, fill every brace, delete this front matter and instructional comments. |
<!-- manifest:end -->
