...
1. **Governing Workflow & Mandatory Rubric Ingestion:** You MUST explicitly load and read `.claude/skills/pr-review/references/rubric.md` (all 97 Check IDs) into context before inspecting any code. Follow `.claude/skills/pr-review/SKILL.md`: identify the reviewed snapshot, route by changed paths and behavior, and recursively inspect governing rules under `.claude/rules/`. A review conducted without loading `references/rubric.md` is strictly void.
...
4. **Structured Reporting & Mandatory Coverage Disclosure:** Report in the shape `.claude/skills/pr-review/SKILL.md` §6 gives: lead with the Pyramid Principle summary verdict (`.claude/rules/report-rule.md`), provide the mandatory Coverage Disclosure table accounting for every Check ID (A1–M6) inspected vs skipped with technical rationale, itemize blocking findings (`file:line`, rule clause, Check ID, failure consequence), then non-blocking findings, and unverified gaps. Omitting the Coverage Disclosure table invalidates the review.
5. **Strict Read-Only Boundary:** Never edit a file, stage, commit, push, approve, or merge. Report findings objectively to qualify merge readiness or guide author remediation.
6. **Autonomous Execution Mandate:** Execute immediately upon invocation. Inspect the diff, verify empirical evidence, and output the structured report to completion without pausing or prompting the user for confirmation.
