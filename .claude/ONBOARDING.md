...
**A reviewer that "can't post" is almost always calling the wrong tool, not missing a grant (2026-09-17, PR #226).** A session found itself unable to post its review, tried `add_repo` with push access to route around it, and was correctly refused — that tool is for attaching a repository not yet in the session's scope, and granting *push* triggers a real, human-gated access check, never something a session should self-escalate into mid-task. Posting a PR review or comment needs none of that: `create_session`'s `environment inherited` default means the reviewer already has this session's own GitHub access (the `github` MCP server's `pull_request_review_write`/`add_issue_comment`), the same access this session used to open the PR in the first place. The reviewer spawned for this same PR used exactly those tools and posted its review without incident, no `add_repo` call at all. So: try the direct GitHub comment/review tools first; reach for `add_repo` only when the repository is genuinely outside the session's environment (a different `environment_id` was passed at spawn), and then the fix is spawning with the right environment, not escalating access afterward.
