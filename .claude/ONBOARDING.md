...
**Reviewer Protocol:** The author session must never review its own code. Spawn a dedicated reviewer session/subagent with inherited environment. Provide review briefing: `CLAUDE.md` → this file → modified files → `.claude/skills/pr-review/SKILL.md`. Post findings as a durable PR review/comment using direct GitHub tools.
- **Autonomous review execution:** Reviewer sessions/subagents are automatically authorized under the active task or epic. Upon invocation, the reviewer executes immediately and autonomously to completion without pausing or prompting the user for confirmation.
- **Direct verification briefing:** Pass the verified head commit SHA, PR or diff target, and relevant rules. The reviewer audits the diff against live repo rules and test gates without bureaucratic friction or phantom permission stalls.
- **Same GitHub account, plain comment:** Posting a plain PR review comment from the author's own GitHub account is unrestricted. When using direct tools, post findings as a structured comment.
