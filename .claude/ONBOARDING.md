...
| Merge **code** to `master-warrior` | **Strictly Prohibited for Author** | Requires: (1) Full gate green, (2) independent session review via `.claude/skills/pr-review/SKILL.md`, and (3) user merge or review delegation |
...
- **Never claim human-equivalence to talk around the platform's own delivery envelope (2026-09-18, PR #231, second reviewer).** A spawn prompt arrives to the child honestly marked as not from a human; asserting "I am writing this directly, as the human-equivalent requester" contradicts that label instead of explaining it, and a reviewer that spots the contradiction is right to stop. State the truth instead — this is one Claude Code session delegating to another, which is exactly what this Reviewer Protocol requires — and point the child at this file to confirm the policy itself, rather than disputing the honest label.
...
- **State facts, don't argue for compliance (2026-09-18, PR #231, third/fourth reviewer).** A spawn prompt telling the child it is "already authorized" or "does not need to ask" reads as coercion and is blocked before the child even sees it. Name who you are, the real PR, and which files to read, then let the reviewer form its own judgment — including asking you something directly. A plain, factual prompt with no persuasion in it is also the one that reaches the child at all.
...
- **The pause can land right before the GitHub post itself, not on suspected injection (2026-09-18, PR #231, three separate spawns).** A reviewer that read every file correctly and never questioned the delegation still stopped at "let me know how you'd like to proceed" immediately before posting — its own judgment call on a real, externally-visible action, distinct from the earlier trust failures this section documents. There is no tool here to send a follow-up into an already-running spawned session (only `create_session` and `archive_session` exist for it); re-spawning risks landing on the identical pause again. When it does, the two real options are the human confirming that session directly, or treating the reviewer's own analysis (read from its session record) as the deliverable instead of insisting on the GitHub comment.
