...
- **The run log is scanned** for `- (WARNING|ERROR|CRITICAL) -` records and any hit fails the run. Every hit is either a real defect (then `fix-bug-rule.md` in full) or an understood, named, expected condition; `-AllowLogWarnings` only triages hits already recorded. "It was there before" means check `Tasks/bug_report/incomplete/`, not skip (`BUG-021`, `BUG-022`). `[gate]`
